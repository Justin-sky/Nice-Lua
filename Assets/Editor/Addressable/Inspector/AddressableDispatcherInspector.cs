...
namespace AssetBundles
{
    ...
    public class AddressableDispatcherInspector : Editor
    {
        public static bool hasAnythingModified = false;

        AddressableDispatcherConfig dispatcherConfig = null;
        ...
        AddressableDispatcherFilterType filterType = AddressableDispatcherFilterType.Root;
        ...
        void DrawCreateAssetBundleDispatcher()
        {
            if (GUILayout.Button("Create AssetBundle Dispatcher"))
            {
                ...
                var instance = CreateInstance<AddressableDispatcherConfig>();
        ...
        void DrawFilterItem(AddressableCheckerFilter checkerFilter)
        {
        ...
        void DrawFilterTypesList(List<AddressableCheckerFilter> checkerFilters)
        {
            ...
            {
                MarkChanged();
                checkerFilters.Add(new AddressableCheckerFilter("", "t:prefab"));
            }
            EditorGUILayout.Separator();
        ...
        void DrawAssetDispatcherConfig()
        {
            ...
            var selectType = (AddressableDispatcherFilterType)EditorGUILayout.EnumPopup(filterType);
        ...
        void ConfirmRemove()
        {
            bool checkRemove = EditorUtility.DisplayDialog("Remove Warning",
                "Sure to remove the Addressable dispatcher ?",
                "Confirm", "Cancel");
            if (!checkRemove)
            {
                return;
        ...
        {
            if (GUILayoutUtils.DrawHeader("Addressable Dispatcher : ", inspectorSate, "DispatcherConfig", true, false))
            {
